...
const ManageSubjects = () => {
    ...
    const fetchSubjects = async () => {
        ...
        try {
            ...
            setSubjects(response.teacherSubjectClasses);
        } catch (error) {
            console.error(error);
        }
    };
    ...
    const openActionSheet = (subjectItem) => {
        ...
        (buttonIndex) => {
            if (buttonIndex === 0) {
                router.push(`/teacher/subjects/${subjectItem.id}`); // Redirect to subject details
            }
            if (buttonIndex === 1) {
    ...
    const renderSubjectItem = ({ item }) => (
        <TouchableOpacity onPress={() => openActionSheet(item.subject)} className="border-b border-gray-300 mx-1 rounded-md p-4 flex-row items-center">
            <View className="flex-1">
                <Text className="font-bold">{item.subject.name}</Text>
                {item?.subject?.description ? <Text>{item.subject.description}</Text> : ''} 
                </View>
        ...
        <View className="flex-1 bg-white">
                ...
                options={{
                    ...
                    headerRight: () => (
                        <TouchableOpacity onPress={() => setModalVisible(false)} className="p-2 rounded-md">
                            <MaterialCommunityIcons name="file-edit" size={24} color="white" />
                        </TouchableOpacity>
                    ),
                }}
            />
          {subjects.length > 0 ?
           <FlatList
            ...
            /> : <Text className="text-md my-4 w-full text-center">No subjects found</Text>
        }
